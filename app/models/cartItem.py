...
class CartItem(Base):
    ...
    cart_id = Column(ForeignKey("carts.id", as_uuid=True), nullable=False)
    cart = relationship(Cart, backref="items", uselist=False)
    
    product_id = Column(UUID(as_uuid=True), nullable=False)  # Assuming products are identified by UUID
